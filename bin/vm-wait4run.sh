...
wait_user_ready4commands() {
    ...
    while [ 1 == 1 ]; do
        ...
        VBoxManage guestcontrol "$VM_DEV_MACHINE" --username "$VM_DEV_USER" --password "$VM_DEV_PASS" run --exe "cmd.exe" -- "cmd.exe" "/c" "copy /b NUL $file"
        sleep 5
        if [ -f "$localfile" ]; then
            break;
        fi
...
wait_user_ready4commands "$file" "$localfile"
echo "Ready ..."
rm -f "$localfile"
